...
SO4 is a keeper venue over unified pool liquidity. Showing an order book layout does **not** create executable resting depth: account orders, oracle prices, and pool balances are not bid/ask level-2 liquidity. Partial fills remain unsupported until the protocol exposes verified fill-quantity events.
